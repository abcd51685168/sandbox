...
BACKUP_FOLDER = "/polydata/image/"
IMG_FOLDER = "/polydata/used_os_image"
...
class LibVirtMachinery(Machinery):
    ...
    def revert_snapshot(self, label, snapshot):
        start_time_sec = datetime.datetime.now()
        self.vms[label].revertToSnapshot(snapshot, flags=0)
        stop_time_sec = datetime.datetime.now()
        diff = stop_time_sec - start_time_sec
        log.info("revert snapshot of %s, cost seconds: %d", label, diff.seconds)
    ...
    def start(self, label):
        ...
        if vm_info.snapshot and vm_info.snapshot in snapshot_list:
            ...
            try:
                ...
                self.revert_snapshot(label, snapshot)
...
class Dumi(object):
    """Base abstract class for domain/url/md5/ip module."""
    order = 1
...
class StaticScan(object):
    """Base abstract class for staticscan module."""
    order = 1
...
class Feed(object):
    ...
    def update(self):
        ...
        """
        self.feedpath = REPORT_ROOT + "/data/feeds/" + self.feedname + ".feed"
