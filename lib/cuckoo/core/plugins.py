...
from lib.cuckoo.common.abstracts import Auxiliary, Machinery, LibVirtMachinery, Processing, StaticScan, Dumi
from lib.cuckoo.common.abstracts import Report, Signature, Feed
from lib.cuckoo.common.config import Config
from lib.cuckoo.common.constants import CUCKOO_ROOT, CUCKOO_VERSION, REPORT_ROOT
from lib.cuckoo.common.exceptions import CuckooCriticalError
from lib.cuckoo.common.exceptions import CuckooOperationalError
from lib.cuckoo.common.exceptions import CuckooProcessingError
from lib.cuckoo.common.exceptions import CuckooReportError
from lib.cuckoo.common.exceptions import CuckooDependencyError
from lib.cuckoo.core.database import Database, DUMI_STARTED, DUMI_FINISHED
from lib.cuckoo.core.database import PROCESSING_STARTED, PROCESSING_FINISHED, SIGNATURES_STARTED, SIGNATURES_FINISHED
from lib.cuckoo.core.database import REPORTING_STARTED, REPORTING_FINISHED, STATIC_SCAN_STARTED, STATIC_SCAN_FINISHED
...
def load_plugins(module):
    for name, value in inspect.getmembers(module):
        if inspect.isclass(value):
            ...
            elif issubclass(value, StaticScan) and value is not StaticScan:
                register_plugin("staticscan", value)
            elif issubclass(value, Dumi) and value is not Dumi:
                register_plugin("dumi", value)
...
class RunAuxiliary(object):
    ...
    def start(self):
        log.info("Task #%d executing auxiliary modules" % self.task.id)
...
class RunProcessing(object):
    ...
    def __init__(self, task_id, results):
        ...
        self.analysis_path = os.path.join(REPORT_ROOT, "storage", "analyses", str(task_id))
    ...
    def run(self):
        ...
        log.info("Task #%d executing processing modules" % self.task["id"])
        ...
        if "behavior" in self.results and "summary" in self.results["behavior"] \
                and "files" in self.results["behavior"]["summary"]:
            Database().set_statistics_counter(self.task_id, DROPPED_FILES,
                                              len(self.results["behavior"]["summary"]["files"]))

        if "behavior" in self.results and "processes" in self.results["behavior"]:
            Database().set_statistics_counter(self.task_id, RUNNING_PROCESSES,
                                              len(self.results["behavior"]["processes"]))
            api_calls = 0
...
class RunSignatures(object):
    ...
    def run(self):
        ...
        log.info("Executing signatures modules")
        ...
        evented_list = [sig(self.results)
                        ...
                        self._check_signature_version(sig) and (
                            not sig.filter_analysistypes or self.results["target"]["category"] in sig.filter_analysistypes)]
        ...
        if complete_list:
            ...
            for signature in complete_list:
                if not signature.filter_analysistypes or self.results["target"]["category"] \
        ...
        for match in matched:
            if "family" in match and match["family"]:
                family = match["family"]
                break
        if not family and "virustotal" in self.results and "results" in self.results["virustotal"] and \
                self.results["virustotal"]["results"]:
            detectnames = []
...
class RunDumi(object):
    """Run dumi Engine.

    This class handles the loading and execution of the static scan modules.
    It executes the enabled ones sequentially and generates a result dictionary.
    """
    ...
    def process(self, module):
        """Run a static scan module.
        @param module: static scan module to run.
        @return: results generated by module.
        """
        # Initialize the specified static scan module.
        try:
            current = module()
        except:
            log.exception("Failed to load the dumi module "
                          "\"{0}\":".format(module))
            return
        ...
        try:
            ...
            log.debug("Dumi module %s not found in configuration file",
                      module_name)
            return None
        ...
        categorys = options.category.split(",")
        if options.category and self.task["category"] not in categorys:
            return None

        # Give it the analysis task object.
        current.set_task(self.task)
        # Give it the options from the relevant staticscan.conf section.
        current.set_options(options)

        try:
            # Run the processing module and retrieve the generated data to be
            # appended to the general results container.
            log.debug("Task #%d start to run %s" % (self.task["id"], current.__class__.__name__))
            data = current.run()
            log.info("Task #%d executed dumi module \"%s\", result of %s is %s"
                      % (self.task["id"], current.__class__.__name__, self.task["target"], {current.key: data}))

            # If succeeded, return they module's key name and the data to be appended to it.
            return {current.key: data}
        except CuckooDependencyError as e:
            log.warning("The dumi module \"%s\" has missing dependencies: %s", current.__class__.__name__, e)
        except CuckooProcessingError as e:
            log.warning("The dumi module \"%s\" returned the following "
                        "error: %s", current.__class__.__name__, e)
        except:
            log.exception("Failed to run the dumi module \"%s\":", current.__class__.__name__)

        return None

    def run(self):
        """Run all static scan modules.
        @return: scan results.
        """
        log.info("Task #%d executing dumi modules" % self.task["id"])
        Database().set_statistics_time(self.task_id, DUMI_STARTED)

        # Order modules using the user-defined sequence number.
        # If none is specified for the modules, they are selected in
        # alphabetical order.
        processing_list = list_plugins(group="dumi")

        # If no modules are loaded, return an empty dictionary.
        if processing_list:
            processing_list.sort(key=lambda module: module.order)
            if self.task["category"] in "file":
                filedetail = File(self.task["target"]).get_all()
                file_md5 = filedetail["md5"]
                self.task["target"] = file_md5
                self.task["category"] = "md5"

            # Run every loaded static scan module.
            for module in processing_list:
                tmp_result = self.process(module)
                if not tmp_result:
                    continue
                self.results = tmp_result
                if self.results.values()[0]:
                    if int(self.results.values()[0][0]) == 1:
                        log.debug("Task #%d %s matched, results is %s"
                                  % (self.task["id"], self.task["target"], self.results))
                        break
                    elif int(self.results.values()[0][0]) == 0 and self.results.keys()[0] in "wbAnalyzer":
                        break
        else:
            log.debug("No dumi modules loaded")
        Database().set_statistics_time(self.task_id, DUMI_FINISHED)
        return self.results


class RunStaticScan(object):
    """Run Static scan Engine.
    ...
    def process(self, module):
        ...
        try:
            ...
            log.exception("Failed to load the static scan module "
                          "\"{0}\":".format(module))
            return
        ...
        try:
            ...
            log.debug("StaticScan module %s not found in configuration file",
                      module_name)
            return None
        ...
        file_type = pd_fmt_file(self.task["target"].encode("utf-8"))[0]
        pseudo_types = options.filetype.strip().split()
        analyzed_types = []
        if pseudo_types:
            for pseudo_type in pseudo_types:
                if "-" in pseudo_type:
                    rangexy = pseudo_type.split("-")
                    analyzed_types += range(int(rangexy[0]), int(rangexy[1]) + 1)
                else:
                    analyzed_types.append(int(pseudo_type))
        else:
            analyzed_types = pseudo_types

        if options.filetype and file_type not in analyzed_types:
            return None
        ...
        try:
            ...
            log.debug("Task #%d executed static scan module \"%s\", result of %s is %s"
                      % (self.task["id"], current.__class__.__name__, self.task["target"], {current.key: data}))

            # If succeeded, return they module's key name and the data to be appended to it.
            return {current.key: data}
        except CuckooDependencyError as e:
            log.warning("The static scan module \"%s\" has missing dependencies: %s", current.__class__.__name__, e)
        except CuckooProcessingError as e:
            log.warning("The static scan module \"%s\" returned the following "
                        "error: %s", current.__class__.__name__, e)
        except:
            log.exception("Failed to run the static scan module \"%s\":", current.__class__.__name__)
    ...
    def run(self):
        ...
        @return: scan results.
        """
        log.info("Task #%d executing staticscan modules" % self.task["id"])
        Database().set_statistics_time(self.task_id, STATIC_SCAN_STARTED)

        # Order modules using the user-defined sequence number.
        # If none is specified for the modules, they are selected in
        # alphabetical order.
        processing_list = list_plugins(group="staticscan")
        file_type = pd_fmt_file(self.task["target"].encode("utf-8"))
        log.info(
            "Task #%d file_type of \"%s\": %s[%s]" % (self.task["id"], self.task["target"], file_type[0], file_type[2]))
        ...
        if processing_list:
            ...
            for module in processing_list:
                ...
                if result:
                    self.results.update(result)
                    # if fast scan and hits signature, return
                    if result.values()[0] and int(self.task["mode"]) == 0:
                        break
        else:
            log.info("No static scan modules loaded")

        Database().set_statistics_time(self.task_id, STATIC_SCAN_FINISHED)
        return self.results
...
class RunReporting:
    ...
    def __init__(self, task_id, results):
        ...
        try:
            del results["sandbox_result"]["behavior"]["processes"]
        except:
            pass
        self.results = results
        self.analysis_path = os.path.join(REPORT_ROOT, "storage", "analyses", str(task_id))
    ...
    def process(self, module):
        ...
        try:
            ...
            log.debug("Task #%d executed reporting module \"%s\"", self.task["id"], current.__class__.__name__)
        except CuckooDependencyError as e:
    ...
    def run(self):
        ...
        log.info("Task #%d executing reporting modules" % self.task["id"])
...
class GetFeeds(object):
    ...
    def run(self):
        ...
        @return None
        """
        log.info("Executing reporting modules")
