...
class ResultServer(SocketServer.ThreadingTCPServer, object):
        ...
        while True:
            try:
                ...
                if e.errno == 98 or e.errno == 48:
                    ...
                    os.system("server_port=`lsof -i:{0} ".format(self.cfg.resultserver.port)
                        + " | awk 'END {{print $2}}'`;if [ ${server_port} ];then kill -9 ${server_port}; sleep 3; fi")
                    os.system("killall tcpdump;sleep 3")
                else:
    ...
    def build_storage_path(self, ip):
        ...
        return os.path.join(REPORT_ROOT, "storage", "analyses", str(task.id))
