...
import modules.staticscan
import modules.dumi

from lib.cuckoo.common.colors import red, green, yellow, cyan
from lib.cuckoo.common.config import Config
from lib.cuckoo.common.constants import CUCKOO_ROOT, CUCKOO_VERSION, REPORT_ROOT, LOG_ROOT
from lib.cuckoo.common.exceptions import CuckooStartupError
from lib.cuckoo.common.exceptions import CuckooOperationalError
from lib.cuckoo.common.utils import create_folders
from lib.cuckoo.core.database import Database, TASK_RUNNING, TASK_FAILED_ANALYSIS, TASK_PENDING
from lib.cuckoo.core.database import TASK_REPORTED, TASK_COMPLETED, TASK_FAILED_REPORTING
from lib.cuckoo.core.plugins import import_plugin, import_package, list_plugins
from lib.cuckoo.core.plugins import GetFeeds, RunProcessing, RunSignatures, RunReporting
...
def check_working_directory():
    ...
    cwd = os.path.join(os.getcwd(), "sandbox.py")
...
def create_structure():
    ...
    try:
        create_folders(root=REPORT_ROOT, folders=folders)
...
def init_logging():
    ...
    fh = logging.handlers.WatchedFileHandler(os.path.join(LOG_ROOT, 'sandbox.log'))
...
def init_tasks():
    ...
    for task in tasks:
        if cfg.cuckoo.reschedule:
            ...
            task_path = os.path.join(REPORT_ROOT, "storage", "analyses", str(task.id))
            if os.path.exists(task_path):
                shutil.rmtree(task_path, True)
            db.set_status(task.id, TASK_PENDING)
...
def init_modules():
    ...
    # Import all staticscan modules.
    import_package(modules.staticscan)
    import_package(modules.dumi)
...
def cuckoo_clean():
    ...
    # Paths to clean.
    paths = [
        os.path.join(REPORT_ROOT, "db"),
        os.path.join(REPORT_ROOT, "log"),
        os.path.join(REPORT_ROOT, "storage"),
    ]
    ...
    # Delete log
    log_path = os.path.join(LOG_ROOT, 'sandbox.log')
