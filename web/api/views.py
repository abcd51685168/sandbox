...
from lib.cuckoo.common.constants import REPORT_ROOT, CUCKOO_VERSION
...
def tasks_delete(request, task_id):
    ...
    if db.delete_task(task_id):
        resp["error"] = False
        delete_folder(os.path.join(REPORT_ROOT, "storage", "analyses",
                                   "%s" % task_id))
...
def tasks_report(request, task_id, report_format="json"):
    ...
    resp = {}
    srcdir = os.path.join(REPORT_ROOT, "storage", "analyses",
                          "%s" % task_id, "reports")
    ...
    if report_format.lower() in formats:
        report_path = os.path.join(REPORT_ROOT, "storage", "analyses",
                                   "%s" % task_id, "reports",
...
def tasks_iocs(request, task_id, detail=None):
    ...
    if repconf.jsondump.get("enabled") and not buf:
        jfile = os.path.join(REPORT_ROOT, "storage", "analyses",
...
def tasks_pcap(request, task_id):
    ...
    srcfile = os.path.join(REPORT_ROOT, "storage", "analyses", "%s" % task_id,
                          "dump.pcap")
...
def tasks_surifile(request, task_id):
    ...
    srcfile = os.path.join(REPORT_ROOT, "storage", "analyses",
...
def tasks_procmemory(request, task_id, pid="all"):
    ...
    srcdir = os.path.join(REPORT_ROOT, "storage", "analyses", "%s" % task_id,
                          "memory")
...
def get_files(request, stype, value):
    ...
    if stype == "sha256":
        file_hash = value
    sample = os.path.join(REPORT_ROOT, "storage", "binaries", file_hash)
