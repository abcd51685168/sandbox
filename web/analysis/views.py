...
def report(request, task_id):
    ...
    if enabledconf["malheur"]:
        malheur_file = os.path.join(REPORT_ROOT, "storage", "malheur", "malheur.txt")
        classes = dict()
        ourclassname = None
        try:
...
def filereport(request, task_id, category):
    ...
    if category in formats:
        file_path = os.path.join(REPORT_ROOT, "storage", "analyses", str(task_id), "reports", formats[category])
...
def full_memory_dump_file(request, analysis_number):
    file_path = os.path.join(REPORT_ROOT, "storage", "analyses", str(analysis_number), "memory.dmp")
    if os.path.exists(file_path):
        content_type = "application/octet-stream"
        response = HttpResponse(open(file_path, "rb").read(), content_type=content_type)
