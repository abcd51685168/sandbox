...
from lib.cuckoo.common.constants import CUCKOO_VERSION, REPORT_ROOT
...
def tasks_delete(task_id):
    ...
    if task:
        ...
        if db.delete_task(task_id):
            delete_folder(os.path.join(REPORT_ROOT, "storage",
                                       "analyses", "%d" % task_id))
            response["status"] = "OK"
        else:
            return HTTPError(500, "An error occurred while trying to "
...
def tasks_report(task_id, report_format="json"):
    ...
    if report_format.lower() in formats:
        report_path = os.path.join(REPORT_ROOT, "storage", "analyses",
            ...
            srcdir = os.path.join(REPORT_ROOT, "storage",
                                  "analyses", "%d" % task_id)
            s = StringIO()
...
def files_get(sha256):
    file_path = os.path.join(REPORT_ROOT, "storage", "binaries", sha256)
...
def pcap_get(task_id):
    file_path = os.path.join(REPORT_ROOT, "storage", "analyses",
                             "%d" % task_id, "dump.pcap")
    if os.path.exists(file_path):
        response.content_type = "application/octet-stream; charset=UTF-8"
        try:
...
def task_screenshots(task=0, screenshot=None):
    folder_path = os.path.join(REPORT_ROOT, "storage", "analyses", str(task), "shots")
