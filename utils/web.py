...
def parse_tasks(rows):
    ...
    if rows:
        for row in rows:
            ...
            if os.path.exists(os.path.join(REPORT_ROOT, "storage", "analyses", str(task["id"]), "reports", "report.html")):
                task["processed"] = True
...
def download_report(task_id):
    ...
    report_path = os.path.join(REPORT_ROOT, "storage", "analyses", task_id, "reports", "report.html")
...
def get_pcap(task_id):
    ...
    pcap_path = os.path.join(REPORT_ROOT, "storage", "analyses", task_id, "dump.pcap")
...
def get_files(task_id):
    ...
    files_path = os.path.join(REPORT_ROOT, "storage", "analyses", task_id, "files")
    zip_file = os.path.join(REPORT_ROOT, "storage", "analyses", task_id, "files.zip")
