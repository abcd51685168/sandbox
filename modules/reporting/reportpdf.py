...
class ReportPDF(Report):
    ...
    def run(self, results):
        ...
        if os.path.exists("/usr/bin/xvfb-run") and os.path.exists("/usr/bin/wkhtmltopdf"):
            call(["/usr/bin/xvfb-run", "--auto-servernum", "--server-num", "1", "/usr/bin/wkhtmltopdf", "-q",
        ...
        HTML(os.path.join(self.reports_path, "summary-report.html"))\
            .write_pdf(os.path.join(self.reports_path, "report.pdf"))
